...
export default function ParentAssignments({ setActivePage }) {
  ...
  const [allAssignmentsList, setAllAssignmentsList] = useState([]);
  ...
  const loadData = async (isManual = false) => {
    ...
    try {
      ...
      // 2. Fetch Full Assignments
      const asgRes = await ApiService.getAssignments(user?.token);
      if (asgRes && asgRes.success && asgRes.data) {
        setAllAssignmentsList(asgRes.data.assignments || []);
      }
    } catch (err) {
  ...
  // Filter assignments strictly for selected child's enrolled class
  const childAssignments = useMemo(() => {
    const targetClass = String(selectedStudent.class || '9');
    return allAssignmentsList.filter(a => !a.class || String(a.class) === targetClass);
  }, [allAssignmentsList, selectedStudent.class]);

  // Extract available subjects
  const availableSubjects = useMemo(() => {
    const subs = new Set(childAssignments.map(a => a.subject).filter(Boolean));
    return ['ALL', ...Array.from(subs)];
  }, [childAssignments]);

  // Filter assignments by status, subject, and search
  const filteredAssignments = useMemo(() => {
    return childAssignments.filter(a => {
  ...
  }, [childAssignments, selectedStatus, selectedSubject, searchQuery]);
  ...
  if (error && childAssignments.length === 0) {
    return (
  ...
  return (
    <div className="parent-assignments-container animate-fade-in">
      {/* Top Action Bar */}
      ...
      {childrenSummaries.length > 1 && (
        <div className="card child-selector-container" style={{ padding: '14px 16px', borderRadius: '16px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.875rem', fontWeight: 700, color: '#1e293b', marginBottom: '8px' }}>
            ...
            <span>Select Child for Homework & Tasks</span>
          </div>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: isSelected ? '#dbeafe' : '#64748b' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <h1 style={{ fontSize: '1.4rem', fontWeight: 800, margin: '2px 0 4px 0' }}>
              Homework & Submission Tasks
            </h1>
            <p style={{ fontSize: '0.85rem', opacity: 0.92, margin: 0 }}>
              Track due dates and homework tasks assigned to Class {selectedStudent.class}
            </p>
          </div>
      ...
      {filteredAssignments.length === 0 ? (
        <div className="card empty-state" style={{ padding: '36px 0', textAlign: 'center', borderRadius: '16px' }}>
          ...
          <h4 style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1e293b', margin: 0 }}>
            {searchQuery ? 'No matching assignments found' : `No assignments for Class ${selectedStudent.class} currently`}
          </h4>
      ...
      {selectedAssignment && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="par-asg-title">
          <div className="modal-content card" style={{ maxWidth: '480px', width: '100%', position: 'relative', borderRadius: '16px', padding: '24px' }}>
            ...
            <h2 id="par-asg-title" style={{ fontSize: '1.2rem', fontWeight: 800, color: '#0f172a', margin: '4px 0 12px 0' }}>
            ...
            <div style={{ background: '#f8fafc', padding: '12px 14px', borderRadius: '10px', fontSize: '0.8125rem', marginBottom: '14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
              <div>Class: <strong>{selectedAssignment.class || selectedStudent.class} ({selectedAssignment.section || 'A'})</strong></div>
