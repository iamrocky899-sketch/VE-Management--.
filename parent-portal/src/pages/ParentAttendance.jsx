...
export default function ParentAttendance({ setActivePage }) {
  ...
  const childAttendanceRecords = fullAttendance.filter(
    r => String(r.studentId) === String(selectedStudent.studentId)
  );

  // Fallback to recentRecords from summary if full records are empty
  const finalAttendanceRecords = childAttendanceRecords.length > 0
    ? childAttendanceRecords
    : (activeChildSummary?.attendanceSummary?.recentRecords || []);

  const totalWorkingDays = attSummary.totalWorkingDays || finalAttendanceRecords.length;
  const presentDays = attSummary.presentDays || finalAttendanceRecords.filter(r => (r.status || '').toUpperCase() === 'PRESENT').length;
  const absentDays = attSummary.absentDays || finalAttendanceRecords.filter(r => (r.status || '').toUpperCase() === 'ABSENT' || (r.status || '').toUpperCase() === 'LEAVE').length;
  const percentage = attSummary.percentage ?? (totalWorkingDays > 0 ? Math.round((presentDays / totalWorkingDays) * 100) : 0);
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: isSelected ? '#d1fae5' : '#64748b' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <p style={{ fontSize: '0.85rem', opacity: 0.92, margin: 0 }}>
              Class <strong>{selectedStudent.class}</strong> • Section <strong>{selectedStudent.section || 'A'}</strong> • Roll No <strong>{selectedStudent.rollNo}</strong> • Official Attendance Register
            </p>
          </div>
