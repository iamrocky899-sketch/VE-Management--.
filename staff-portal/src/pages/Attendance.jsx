...
export default function Attendance({ onNavigate }) {
  ...
  const loadClassAttendanceData = async (isManual = false) => {
    ...
    try {
      ...
      const subRes = await sendApiRequest('get_subjects', { class: selectedClass });
        ...
        <div>
          ...
          <div className="card" style={{ padding: '16px', marginBottom: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', alignItems: 'center' }}>
              <div>
                ...
                <select className="input-field" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} style={{ height: '38px' }}>
          ...
          {loading ? (
            ...
            <div className="card" style={{ padding: '60px', textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <div style={{ fontSize: '0.85rem' }}>Ensure students are enrolled in Class {selectedClass} Section {selectedSection}.</div>
            </div>
          ) : (
