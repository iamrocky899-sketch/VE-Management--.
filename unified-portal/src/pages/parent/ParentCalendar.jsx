...
import {
  ...
  BookOpen, Award, Sun, Umbrella, Clock, Users
} from 'lucide-react';
...
export default function ParentCalendar({ setActivePage }) {
  ...
  const childClass = String(selectedStudent.class || '9');

  // When switching child, clear day detail
  ...
  // Day-status evaluator (Attendance 2.0 compatible, class-aware Saturday rules)
  const getDayEvaluation = (dateStr) => {
    ...
    if (dayOfWeek === 0) {
      return { status: 'SUNDAY', title: 'Sunday (Weekend Off)', isWorking: false };
    }

    // 3. Saturday Rule: Class 9-10 OFF, Class 11-12 Working (child-class aware)
    if (dayOfWeek === 6) {
      if (childClass === '9' || childClass === '10') {
        return { status: 'SATURDAY_OFF', title: 'Saturday Off (Class IX–X)', isWorking: false };
      }
      return { status: 'SATURDAY_WORKING', title: 'Saturday (Working Day)', isWorking: true };
    }

    // 4. Default Working Day
    return { status: 'WORKING_DAY', title: 'Working Day', isWorking: true };
  };
  ...
  const monthDays = useMemo(() => {
    ...
    for (let i = firstDayIndex - 1; i >= 0; i--) {
      ...
      days.push({ dayNum, dateStr, isCurrentMonth: false, evaluation: evaluateCalendarDate(dateStr, childClass, calendarEvents, []) });
    }
    ...
    for (let dayNum = 1; dayNum <= daysInMonth; dayNum++) {
      ...
      days.push({ dayNum, dateStr, isCurrentMonth: true, evaluation: evaluateCalendarDate(dateStr, childClass, calendarEvents, []) });
    }
    ...
    return days;
  }, [currentYear, currentMonth, calendarEvents, childClass]);
  ...
  const monthSummary = useMemo(() => {
    const daysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate();
    let workingDays = 0, holidays = 0, events = 0;
    for (let d = 1; d <= daysInMonth; d++) {
      const monthStr = String(currentMonth + 1).padStart(2, '0');
      const dStr = String(d).padStart(2, '0');
      const dateStr = `${currentYear}-${monthStr}-${dStr}`;
      const ev = evaluateCalendarDate(dateStr, childClass, calendarEvents, []);
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8rem', opacity: 0.9, margin: '2px 0 0 0' }}>
              {selectedStudent.studentName} • Class {childClass} • Session 2026–27
            </p>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div style={{
              ...
              <p style={{ fontSize: '0.78rem', color: '#94a3b8', marginTop: '8px', margin: '8px 0 0 0' }}>
                Student: {selectedStudent.studentName} (Class {childClass})
              </p>
            </div>
