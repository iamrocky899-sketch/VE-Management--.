...
import {
  ...
  AlertTriangle, Info, Calendar, ChevronRight, Megaphone
} from 'lucide-react';
...
export default function StudentNotices({ setActivePage }) {
  ...
  const loadNotices = async (isManual = false) => {
    ...
    try {
      const res = await ApiService.getNotices(user?.token);
      if (res && res.success && res.data) {
        const notices = res.data.notices || [];
        // Sort: Highlighted first, then URGENT, then IMPORTANT, then by newest date
        notices.sort((a, b) => {
      ...
      } else if (res?.error?.code === 'SESSION_EXPIRED') {
        handleSessionRevocation('SESSION_EXPIRED');
        return;
      } else if (res?.error?.code === 'ACCOUNT_DEACTIVATED') {
        handleSessionRevocation('ACCOUNT_DEACTIVATED');
        return;
      } else {
        setError(res?.error?.message || 'Unable to load notices.');
      }
    } catch (err) {
  ...
  // Filter notices by priority and search
  ...
  }, [noticesList, selectedPriority, searchQuery]);
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8rem', opacity: 0.9, margin: '2px 0 0 0' }}>
              Class {user?.class || '—'} • {noticesList.length} notice{noticesList.length !== 1 ? 's' : ''} available
            </p>
          </div>
        </div>
      </div>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap', gap: '6px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    {(notice.isHighlighted === true || notice.isHighlighted === 'true' || notice.highlighted === true) && (
                      <span style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: '4px',
